...
class Instagram extends EmbedParserAbstract implements EmbedParserCustomInterface
{
    public function regex()
    {
        return [
            /**
             * The following URLs are derived from instagram oembed definitions:
             *
             * [
            ...
            // direct post link
            "https?://(?:www\.)?(?:instagram\.com|instagr.am)/(?P<type>p|tv|reel)/(?P<id>[a-zA-Z0-9_-]+)",
            // user profile post link
            "https?://(?:www\.)?(?:instagram\.com|instagr.am)/(?P<username>[a-zA-Z0-9_]+)/(?P<type>p|tv|reel)/(?P<id>[a-zA-Z0-9_-]+)",
        ];
    }
    ...
    public function getEmbedHtml(array $matches): string
    {
        ...
        $postId = $matches['id'] ?? '';
        $url = "https://www.instagram.com/$type/{$postId}/?utm_source=ig_embed&utm_campaign=loading";

        $username = $matches['username'] ?? null;
        $caption = $username ? "A post shared by @{$username}" : '';
